...
dataDir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'training_data/')
resDir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'training_results/')
...
X = np.load(dataDir+'spec_'+filename+'.npy')
Y = np.load(dataDir+'geom_'+filename+'.npy')
...
np.savetxt(resDir+filename+'_MAE.dat', MAE)
"""
print('Done.')
